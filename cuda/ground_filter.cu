...
// 原子操作可用于创建无（互斥）锁算法，极大提升并行性能，尤其在高竞争场景下
// GPU 仅此一种方法可以实现（自旋）锁，这种锁会让线程进行等待，但是切换非常高效
// 下面是一个简单的自旋锁实现，虽然在这个项目中我们不需要它，但它是理解 GPU 原子操作的好例子
// class GpuSpinLock {
//     int* mutex; // 指向显存中的一个 int，初始化为 0 (解锁状态)

// public:
//     __device__ GpuSpinLock(int* m) : mutex(m) {}

//     __device__ void lock() {
//         // while 循环：这就是“死死盯着桌子”
//         // atomicCAS(mutex, 0, 1) 的意思是：
//         // 如果 mutex 是 0 (空闲)，把它改成 1 (上锁)，并返回 0 -> 循环结束，拿到锁！
//         // 如果 mutex 是 1 (被别人占了)，它返回 1 -> 1 != 0，条件成立 -> 继续循环，重试。
//         while (atomicCAS(mutex, 0, 1) != 0) {
//             // 这里什么都不做，就是空转 (Busy Wait)
//             // 有时会加一个 __threadfence_block() 防止编译器优化过度
//         }
//     }

//     __device__ void unlock() {
//         // 释放锁很简单，直接原子地把 1 改回 0
//         // 使用 atomicExch (交换) 是为了保证这是一个原子写操作
//         atomicExch(mutex, 0); 
//     }
// };
